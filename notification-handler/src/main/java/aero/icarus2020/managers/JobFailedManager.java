...
@Service
public class JobFailedManager extends Manager {
    ...
    public void sendNotification() throws InterruptedException, ExecutionException, MissingAttributeException {
        System.out.println("Processing 'job failed' event...");
            ...
            if (notifications != null)
                for (NotificationsDao notification : notifications)
                    if (notification.getLabel().compareToIgnoreCase("analytics") == 0) {
                        ...
                        buildNotification(notification, ownerUser, meta, content, "JOB_FAILED", "Analytics Job Failed");
                        break;
                    }
        }


        System.out.println("Processing 'job failed' event...Done");
    }
