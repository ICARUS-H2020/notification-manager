...
@Service
public class ContractOfferedManager extends Manager {
    ...
    public void sendNotification() throws InterruptedException, ExecutionException, MissingAttributeException {
        System.out.println("Processing 'contract offered' event...");
            ...
            if (notifications != null)
                for (NotificationsDao notification : notifications)
                    if (notification.getLabel().compareToIgnoreCase("contracts") == 0) {
                        ...
                        buildNotification(notification, orgMan, meta, content, "CONTRACT_OFFERED", "Contract Offered");
                        break;
                    }
        }

        System.out.println("Processing 'contract offered' event...done");
    }

    private void sendNotificationToApplicantManager() throws ExecutionException, InterruptedException {
        ...
        wn.setNotif_type("CONTRACT_OFFERED");
        wn.setRecipient_id(orgMan.getId());
        wn.setMeta(meta);
