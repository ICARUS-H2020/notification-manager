...
@Service
public class ContractOfferRejectedManager extends Manager {
    ...
    public void sendNotification() throws InterruptedException, ExecutionException, MissingAttributeException {
        System.out.println("Processing 'contract offer rejected' event...");
            ...
            if (notifications != null)
                for (NotificationsDao notification : notifications)
                    if (notification.getLabel().compareToIgnoreCase("contracts") == 0) {
                        ...
                        buildNotification(notification, orgMan, meta, content, "CONTRACT_OFFER_REJECTED", "Contract Offer Rejected");
                        break;
                    }
        }

        System.out.println("Processing 'contract offer rejected' event...done");
    }
