...
@Service
public class DatasetUpdateFailedManager extends Manager {
    ...
    public void sendNotification() throws InterruptedException, ExecutionException, MissingAttributeException {
        System.out.println("Processing 'dataset update job failed' event...");
            ...
            if (notifications != null)
                for (NotificationsDao notification : notifications)
                    if (notification.getLabel().compareToIgnoreCase("data-check-in") == 0) {
                        ...
                        buildNotification(notification, userOwner, meta, content, "DATA_ASSET_UPDATE_FAILED", "Data Asset Update Failed");
                        break;

                    }
        }

        System.out.println("Processing 'dataset update job failed' event...done");
    }
