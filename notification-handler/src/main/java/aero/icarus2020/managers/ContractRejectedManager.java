...
@Service
public class ContractRejectedManager extends Manager {
    ...
    public void sendNotification() throws InterruptedException, ExecutionException, MissingAttributeException {
        System.out.println("Processing 'contract rejected' event...");
            ...
            if (notifications != null)
                for (NotificationsDao notification : notifications)
                    if (notification.getLabel().compareToIgnoreCase("contracts") == 0) {
                        ...
                        buildNotification(notification, orgMan, meta, content, "CONTRACT_REJECTED", "Contract Rejected");
                        break;
                    }
        }
        System.out.println("Processing 'contract rejected' event...done");
    }
