...
@Service
public class JobCompletedManager extends Manager {
    ...
    public void sendNotification() throws InterruptedException, ExecutionException, MissingAttributeException {
        System.out.println("Processing 'job completed' event...");
            ...
            if (notifications != null)
                for (NotificationsDao notification : notifications)
                    if (notification.getLabel().compareToIgnoreCase("analytics") == 0) {
                        ...
                        buildNotification(notification, ownerUser, meta, content, "JOB_COMPLETED", "Analytics Job Completed");
                        break;
                    }
        }


        System.out.println("Processing 'job completed' event...Done");
    }
